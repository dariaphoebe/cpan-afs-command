...
=head1 SYNOPSIS
    ...
    my $fs = AFS::Command::FS->new( command => $path_to_your_fs_binary );

=head1 DESCRIPTION

This module implements an OO API wrapper around the AFS 'fs' command.
The supported methods depend on the version of the fs binary used, and
are determined automagically.
...
Exception handling when using the path methods can be very tedious.
In many cases, you either want the AFS::Object::Path object for a
given path, or you want to raise an exception.  Due to the way the
path command methods are implemented to support querying multiple
paths, you end up with code that looks like this:

    my $cm = $fs->listacl( path => $path );
    my $pathobj = $cm->getPath( $path );
    if ( $pathobj->error ) {
        die qq{ARGHH!!!\n} . $pathobj->error;
    }

A simple wrapper method can be used to accomplish this, as follows:

    my $pathobj = $fs->getpathinfo( method => q{listacl}, path => $path );

The getpathinfo method takes one additional key/value pair in addition
to the arguments of the method being wrapped, and that is the method
being wrapped.  The rest of the arguments are passed to that method
as-is.  However, getpathinfo can only be called for a single path.
Passing an ARRAY reference will result in an exception.

The return value is the AFS::Object::Path object for the given path.

=head1 METHODS (path commands)
...
getcalleraccess, getfid, listacl, listquota, quota, storebehind,
whereis, and whichcell.

All of these commands return an AFS::Object::CacheManager object,
which contains one or more AFS::Object::Path objects, one for each
path specified in the arguments.  Each of these objects has the
...
This method returns an AFS::Object::CacheManager object, which is
documented above.
    ...
    foreach my $pathobj ( $result->getPaths ) {
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            print qq{Path '$path' has errors '} . $pathobj->error . qq{'\n};
        } else {
            foreach my $attr ( qw( volname used total avail percent ) ) {
                print qq{Path '$path' has '$attr' of '} . $pathobj->$attr . qq{'\n};
            }
        }
    }
...
The fs help string is:
    ...
    Usage: fs getcalleraccess [-path <dir/file path>+]

The corresponding method invocation looks like:

    my $result = $fs->getcalleraccess(
       # Optional arguments
       path => $path, # OR [ $path1, $path2, ... ]
    );
...
This method returns an AFS::Object::CacheManager object, which is documented above.
...
If no errors were encountered, then the following attributes will be present:
    ...
    ----------                  ------
    path                        The pathname
    rights                      Caller's ACL rights on that path
...
If no errors were encountered, then the following attributes will be present:
    ...
    ----------                  ------
    path                        The pathname
    volume                      The volume ID for that path
    vnode                       The FID vnode for that path
    unique                      The FID uniquifier for that path
    cell                        The cell for that path[*]

NOTE: The errors returned will almost always be "Unable to determine
results".  As of OpenAFS 1.5.77, the getfid command returns no errors
at all when it can not determine the FID, and the exit code is always
zero.

[*] This is only available in very recent versions of OpenAFS, since
getfid was not modified to display the cell until late in 2010.  For a
more reliable mechanism, use whichcell, which works with all existing
fs binaries.
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            ...
            foreach my $type ( qw( normal negative ) ) {
                ...
                foreach my $principal ( keys %entries ) {
                    ...
                    print qq{$type rights for $principal are $rights\n};
                }
            }
        }
    }

Each AFS::Object::Path object contains one or two AFS::Object::ACL
objects (one for normal, and one for negative).
...
documented above.
    ...
    foreach my $pathobj ( $result->getPaths ) {
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            ...
            print qq{Path '$path' has quota '} . $pathobj->quota . qq{'\n};
        }
    }
...
In addition to the usual methods available on the CacheManager object,
the objects returned by this method will also have the following
attribute as well:
...
documented above.
    ...
    foreach my $pathobj ( $result->getPaths ) {
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            ...
            print qq{Path '$path' is on hosts } . join( q{ ,}, @{ $pathobj->hosts} ) . qq{\n};
        }
    }
...
documented above.
    ...
    foreach my $pathobj ( $result->getPaths ) {
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            ...
            print qq{Path '$path' is in cell '} . $pathobj->cell . qq{'\n};
        }
    }
...
The fs help string is:
    ...
    Usage: fs bypassthreshold [-size <file size>]

The corresponding method invocation looks like:

    my $result = $fs->bypassthreshold(
        # Optional arguments
        size => $size,
    );
...
This method returns an AFS::Object::CacheManager object with one attribute.
    ...
    Attributes                  Values
    ----------                  ------
    bypassthreshold             The value of the cache bypass threshold    

Note that a value of -1 means disabled.
...
contains one or more attributes.
    ...
    foreach my $server ( $result->servers ) {
        print qq{Server $server appears to be down\n};
    }
...
Note that the interval attribute is only present if the interval
...
more attributes.
    ...
    foreach my $attr ( qw( convert uidcheck submounts ) ) {
        print qq{Translator has '$attr' set to '} . $result->$attr . qq{'\n};
    }
...
more attributes.
    ...
    print qq{Cache is using $used KB of $avail KB available\n};
...
one or more AFS::Object::Cell objects.
    ...
    foreach my $cellobj ( $result->getCells ) {
        ...
        if ( $cellobj->status ) {
            print qq{This client allows setuid binaries from cell '$cell'\n};
        } else {
            print qq{This client does NOT allow setuid binaries from cell '$cell'\n};
        }
    }
...
This method returns an AFS::Object::CacheManager object with one attribute.
    ...
    print qq{This client has the following addressed configured for AFS:\n};
...
This method returns an AFS::Object::CacheManager object with one attribute.
    ...
    my $result = $fs->getcrypt
    print qq{This client has encryption turned } . ( $result->crypt ? q{on} : q{off} ) . qq{\n};
...
one or more AFS::Object::Server objects.
    ...
    foreach my $serverobj ( $result->getServers ) {
        ...
        print qq{Server '$server' has preference '$preference'\n};
    }
...
contains one or more AFS::Object::Cell objects.
    ...
    foreach my $cellobj ( $result->getCells ) {
        ...
        print qq{Cell '$cell' has alias '$alias'\n};
    }
...
contains one or more AFS::Object::Cell objects.
    ...
    foreach my $cellobj ( $result->getCells ) {
        ...
        print qq{Cell $cell has servers } . join( q{ }, @{$servers} ) . qq{\n};
    }
...
specified in the arguments.
    ...
    foreach my $pathobj ( $result->getPaths ) {
        ...
        if ( $pathobj->hasAttribute( q{error} ) ) {
            ...
            print(
                qq{Path '$path' is a mtpt for volume $volname} .
                  ( $cell ? qq{, in cell '$cell'\n} : qq{\n} )
            );
        }
    }
...
of two possible attributes.
    ...
    print qq{This client has a primary sysname of '$sysname'\n};
    if ( ref $sysnames eq q{ARRAY} ) {
        print qq{This client has a list of sysnames: } . join( q{ ,}, @{ $sysnames } ) . qq{\n};
    }
...
NOTE: The -generate option is optional only for OpenAFS versions 1.5.8
or newer, and the uuid command was only introduced in OpenAFS 1.4.5.
For versions prior to 1.5.8, this command will return a true/false
value, but for 1.5.8 and beyond, it return an object containing the
uuid value.

=item Return Values

The return value for this method depends on the version of the fs
command being used.  See above for the version details.  If the fs
command returns the uuid, then this method returns an
AFS::Object::CacheManager object which has one attribute.

    Attributes                  Values
    ----------                  ------
    uuid                        The UUID of the AFS client

If the fs command does B<NOT> return the uuid, then this method simply
returns a true/false value, not an object.  This can be handled
programmatically as follows:
    ...
    if ( $fs->supportsOperation( q{uuid} ) {
        if ( $fs->supportsArgumentRequired( qw( uuid generate ) ) {
            ...
            my $result = $fs->uuid;
            my $old_uuid = $result->uuid;
            $result = $fs->uuid( generate => 1 );
            my $new_uuid = $result->uuid;
            warn qq{UUID changed from $old_uuid to $new_uuid\n};
        }
    } else {
        die qq{Unable to set the uuid\n};
    } 
...
attribute.
    ...
    my $result = $fs->wscell;
    print qq{This client lives in cell '} . $result->cell . qq{'\n};
...
The fs help string is:
    ...
    Usage: fs discon -mode <offline | online> [-policy <client | server>]
                     [-force] [-uid <Numeric UID of user whose tokens to use at reconnect.>]
...
The fs help string is:
    ...
    Usage: fs nukenfscreds -addr <host name or address>

The corresponding method invocation looks like:

    my $result = $fs->nukenfscreds(
       # Required arguments
       addr => $addr,
    );
...
pass AFS::Object::ACL objects as arguments to these API calls, but not
yet...
...
The fs help string is:
    ...
    Usage: fs setcbaddr [-addr <host name or address>]

The corresponding method invocation looks like:

    my $result = $fs->setcbaddr(
       # Optional arguments
       addr => $addr,
    );
