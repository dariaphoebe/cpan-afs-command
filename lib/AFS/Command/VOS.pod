...
=head1 SYNOPSIS
    ...
    my $vos = AFS::Command::VOS->new( command => $path_to_your_vos_binary );
...
This method is a simple shorthand for the following common use case:

    my $result = $vos->listvldb( name => $name, cell => $cell );
    my $vldbentry = $result->getVLDBEntry( name => $name );

When querying the VLDB entry for a single volume, the VLDBEntry object
can be queried directly using this wrapper method instead:

    my $vldbentry = $vos->getVLDBEntry( name => $name, cell => $cell );

The name is a required argment, and it must be a scalar, not an ARRAY
reference.
...
The vos help string is:
    ...
    Usage: vos dump -id <volume name or ID> [-time <dump from time>] [-file <dump file>]
                    ...
                    [-clone] [-omitdirs] [-cell <cell name>]
                    [-noauth] [-localauth] [-encrypt] [-noresolve]
...
be given as follows:
    ...
    my $result = $vos->dump(
       ...
       filterout => [qw( newversion -arg1 -value1 )],
       ...
    );
...
specified as follows:
    ...
    my $result = $vos->dump(
       ...
       filterout => [
           [qw( command1 -arg1 -value1 )],
           [qw( command2 -arg2 -value2 )],
           [qw( command3 -arg3 -value3 )],
       ],
       ...
    );
...
be given as follows:
    ...
    my $result = $vos->restore(
       ...
       filterin => [qw( newversion -arg1 -value1 )],
       ...
    );
...
This method returns a generic AFS::Object with the following attributes:

    Attributes                  Values
    ----------                  ------
    total                       Number of volumes successfully backed up
    failed                      Number of volumes failed
    volumes                     ARRAY reference of volume names backed up (or attempted)

If the -dryrun argument is given, then the total and failed counts
will both be zero, since no actual backups will be performed.
...
The vos help string is:
    ...
    Usage: vos examine -id <volume name or ID> [-extended] [-format]
                       [-cell <cell name>] [-noauth] [-localauth]
                       [-encrypt] [-noresolve]
...
NOTE: The -format option is NOT supported, since AFS::Command is
designed to parse the default output, not the machine parseable output
generated by -format.  Use of -format will result in an exception.
...
This method returns a false value if the volume name is not present in
the VLDB, and raises an exception for all other errors.
    ...
    foreach my $header ( $result->getVolumeHeaders ) {
        ...
        print qq{[header] server = $server, partition = $partition\n};
    }
    ...
    foreach my $vldbsite ( $vldbentry->getVLDBSites ) {
        ...
        print qq{[vldbsite] server = $server, partition = $partition\n};
    }
...
The vos help string is:
    ...
    Usage: vos listaddrs [-uuid <uuid of server>] [-host <address of host>]
                         [-printuuid] [-cell <cell name>] [-noauth]
                         [-localauth] [-encrypt] [-noresolve]
...
This method returns a list of AFS::Object::FileServer objects, each of
which has attributes that vary depending on the arguments to the
method call.
...
course, but that is outside of vos' control).
    ...
    my @result = $vos->listaddrs( cell => $cell );

    foreach my $result ( @result ) {
        if ( $result->hasAttribute( q{hostname} ) {
            print qq{Hostname: } . $result-hostname . qq{\n};
        } elsif ( $result->hasAttribute( q{addresses} ) {
            foreach my $address ( $result->addresses ) {
                print qq{IP Address: $address\n};
            }
        }
    }
...
The vos help string is:
    ...
    Usage: vos listpart -server <machine name> [-cell <cell name>]
                        [-noauth] [-localauth] [-encrypt] [-noresolve]
...
This method returns an AFS::Object::FileServer object, which contains
one or more AFS::Object::Partition objects.  Because 'listpart'
returns nothing other than the partition names, the underlying
Partition objects have only one attribute ('partition'), so the API
for access this data is trivial:

    my $result = $vos->listpart(
       server => q{fs1.example.com},
    );

    foreach my $partition ( $result->getPartitionNames ) {
        print qq{Server '$server' has partition '$partition'\n};
    }
...
This method returns an AFS::Object::VLDB object, which has a few
attributes, and contains one or more AFS::Object::VLDBEntry objects,
which in turn contain AFS::Object::VLDBSite objects, as well as their
own attributes.
...
relevant details on the parsing of those objects, which will not be
repeated here.

    my $result = $vos->listvldb( cell => $cell );

    print( q{VLDB contains } . $result->total . q{ volumes, } .
           $result->locked . qq{ of which are locked\n});

    foreach my $entry ( $result->getVLDBEntries ) {
        my $name = $entry->name;
        foreach my $attr ( $entry->listAttributes ) {
            print qq{Volume $name has attribute $attr => } . $entry->$attr . qq{\n};
        }
        foreach my $site ( $entry->getVLDBSites ) {
            my %attrs = $site->getAttributes;
            while ( my($attr,$value) = each %attrs ) {
                print qq{Site has attribute $attr => $value\n};
            }
        }
    }
...
The vos help string is:
    ...
    Usage: vos listvol -server <machine name> [-partition <partition name>]
                       [-fast] [-long] [-quiet] [-extended] [-format]
                       [-cell <cell name>] [-noauth] [-localauth]
                       [-encrypt] [-noresolve]
...
This method returns an AFS::Object::VolServer object, which merely
contains one or more AFS::Object::Partition objects, which in turn
have a few attributes and contain one or more
AFS::Object::VolumeHeader objects.
    ...
    foreach my $partition ( $result->getPartitions ) {
        ...
        print( qq{Partition $partname has $total total volumes, of which } .
               qq{$online are online, $offline are offline, and $busy are busy.\n});
...
The vos help string is:
    ...
    Usage: vos partinfo -server <machine name> [-partition <partition name>]
                        [-summary] [-cell <cell name>] [-noauth] [-localauth]
                        [-encrypt] [-noresolve]
...
NOTE: if supported, the -summary argument is passed by default, so it
never needs to be specified at all.  If your vos binary support
-summary, then you will get the summary information for free.

=item Return Values

This method returns an AFS::Object::FileServer object, which contains
one or more AFS::Object::Partition objects, which have more
interesting attributes than those returned by 'vos partinfo'.
    ...
    if ( $vos->supportsArgument( q{partinfo}, q{summary} ) {
        my $partitions = $result->partitions;
        my $available  = $result->available;
        my $total      = $result->total;
        print( qq{Server $server has $available KB of } .
               qq{space available out of $total KB total } .
               qq{on $partitions partitions\n} );
    }
    ...
    foreach my $partition ( $result->getPartitions ) {
        ...
        my $total     = $partition->total;
        print( qq{Partition $partname has $available KB of } .
               qq{space available out of $total KB total\n} );
    }

B<AFS::Object::FileServer>

This object has no attributes, unless the API is used with a vos
binary that supports the -summary option.  If -summary is supported,
then this object has 3 attributes:

    Attributes                  Values
    ----------                  ------
    partitions                  Number of partitions counted in the summary
    available                   Space available, in KB
    total                       Total space, in KB

This object is primarily a container for the AFS::Object::Partition
objects.  It has the following methods for extracting the objects is
contains.
...
The vos help string is:
    ...
    Usage: vos size -id <volume name or ID>
                    [-partition <partition name>] [-server <machine name>]
                    [-dump] [-time <dump from time>] [-cell <cell name>]
                    [-noauth] [-localauth] [-encrypt] [-noresolve]
...
NOTE: Because calling "vos size" without -dump is meaningless, the
value of the dump option defaults to true.  IOW, you do not need to
pass dump => 1 to the method.  

=item Return Values

This method returns a generic AFS::Object object with two attributes:

    Attributes                          Values
    ----------                          ------
    volume                              Name of the volume
    dump_size                           Size of the dump
...
AFS::Object::Transaction objects.
    ...
    print q{Server has } . $result->transactions . qq{ active transactions\n};

    foreach my $transaction ( $result->getTransactions ) {
        print( q{There are active transactions for volume ID } .
               $transaction->volume . qq{\n});
    }
...
All of the following commands return a true value if they succeed, and
...
The vos help string is:
    ...
    Usage: vos addsite -server <machine name for new site> -partition <partition name for new site>
                       -id <volume name or ID> [-roid <volume name or ID for RO>]
                       [-valid] [-cell <cell name>] [-noauth] [-localauth]
                       [-encrypt] [-noresolve]
...
The vos help string is:
    ...
    Usage: vos clone -id <volume name or ID> [-server <server>] [-partition <partition>]
...
The vos help string is:
    ...
    Usage: vos convertROtoRW -server <machine name> -partition <partition name>
                             -id <volume name or ID> [-force] [-cell <cell name>]
                             [-noauth] [-localauth] [-encrypt] [-noresolve]
...
The vos help string is:
    ...
    Usage: vos copy -id <volume name or ID on source>
...
The vos help string is:
    ...
    Usage: vos delentry [-id <volume name or ID>+]
                        ...
                        [-noexecute] [-cell <cell name>] [-noauth] [-localauth]
                        [-encrypt] [-noresolve]
...
The vos help string is:
    ...
    vos endtrans: end a volserver transaction 
    Usage: vos endtrans -server <machine name> -transaction <transaction ID>
                        [-cell <cell name>] [-noauth] [-localauth]
                        [-encrypt] [-noresolve]

The corresponding method invocation looks like:

    my $result = $vos->endtrans(
       # Required arguments
       server      => $server,
...
The vos help string is:
    ...
    vos online: force the volume status to online 
...
The vos help string is:
    ...
    vos setaddrs: set the list of IP address for a given UUID in the VLDB 
    Usage: vos setaddrs -uuid <uuid of server> -host <address of host>+
                        [-cell <cell name>] [-noauth] [-localauth]
                        [-encrypt] [-noresolve]
...
The vos help string is:
    ...
    Usage: vos setfields -id <volume name or ID> [-maxquota <quota (KB)>]
                         [-clearuse] [-clearVolUpCounter] [-cell <cell name>]
                         [-noauth] [-localauth] [-encrypt] [-noresolve]
...
The vos help string is:
    ...
    Usage: vos shadow -id <volume name or ID on source>
                      -fromserver <machine name on source>
                      -frompartition <partition name on source>
                      -toserver <machine name on destination>
                      -topartition <partition name on destination>
...
NOTE: Since this operation returns no data, use of the -dryrun option,
while "supported", is meaningless.  This method will need to be
converted to one which returns objects encapsulating that data before
-dryrun can be used to query what would happen.
...
The vos help string is:
    ...
    Usage: vos syncvldb [-server <machine name>] [-partition <partition name>]
                        [-volume <volume name or ID>] [-dryrun]
                        [-cell <cell name>] [-noauth] [-localauth]
                        [-encrypt] [-noresolve]
